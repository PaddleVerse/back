...
export class ChatGateway {
  ...
  //     const u = await this.userService.getUserById(
  //       +socket.handshake.query.userId
  //     );
  //     if (!u)
  //       throw new Error(
  //         `the user with the id ${socket.handshake.query.userId} doesn't exist`
  //       );
  ...
  //       throw new Error(`the user doesnt have privilege over the channel`);
  ...
  //     });
  //     if (!ch) throw new Error("update was not applied");
  ...
  // async newMessage_Group(
  //   @ConnectedSocket() socket: Socket,
  //   @Body("message") message: string,
  //   @Body("channel") channelName: string
  // ) {
  //   try {
